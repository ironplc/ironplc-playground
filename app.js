...
function isRuntimeReportable(code) {
...
function reportKind(reportable) {
...
function overrunDiagnostic(message) {
    return {
...
function extractErrorLocations(diagnostics) {
    if (!diagnostics)
        return [];
    const seen = new Set();
    const locations = [];
    for (const d of diagnostics) {
        if (!isReportable(d.code) || !d.compiler_file)
            continue;
...
function initDialects(options) {
    ...
    if (allowsParam.length > 0) {
        ...
        const dialectLabel = dialectSelect.options[dialectSelect.selectedIndex]?.textContent ??
            "default";
...
if (params.has("code")) {
    try {
        ...
        if (params.get("scaffold") === "true") {
            ...
            const startsWithPOU = /^(PROGRAM|FUNCTION_BLOCK|FUNCTION)\s/i.test(trimmed);
            if (!startsWithPOU) {
                let varBlock = "";
                if (params.has("vars")) {
...
function startStepLoop() {
    ...
    stepIntervalId = window.setInterval(async () => {
        ...
        if (elapsed > intervalMs) {
            ...
            const message = `Cycle overrun: program execution took ${elapsed.toFixed(0)}ms but the ` +
...
function renderDiagnostics(diagnostics) {
    ...
    const reportable = diagnostics.filter((d) => isReportable(d.code) || isRuntimeReportable(d.code));
...
const REPORT_CONSENT_HTML = "Submitting sends the program in the editor to the IronPLC team so we can fix " +
    "it. <strong>Your code may be published publicly</strong> — for example in a " +
    "GitHub issue. Don’t submit anything confidential.";
function reportPanelHtml(reportable) {
    const codes = extractErrorCodes(reportable).join(", ");
    const title = reportKind(reportable) === "runtime"
    ...
    return ('<div class="report-panel" data-testid="report-panel">' +
...
function wireReportPanel(reportable) {
    const btn = diagnosticsPanel.querySelector(".submit-code-btn");
    if (!btn)
        return;
...
function submitCodeReport(reportable) {
...
function buildGithubIssueUrl(reportable) {
    const source = lastCompiledSource;
    const codes = extractErrorCodes(reportable);
    const config = `**Compiler version:** ${compilerVersion || "unknown"}\n` +
    ...
    const withProgram = header + `\n**Program**\n\`\`\`iecst\n${source}\n\`\`\`\n`;
    const withoutProgram = header +
